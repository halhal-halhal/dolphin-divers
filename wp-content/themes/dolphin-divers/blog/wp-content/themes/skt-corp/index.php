...
<?php else: ?>
    ...
    if( of_get_option('numsection', true) > 0 ) { 
        ...
        for( $s=1; $s<=$numSections; $s++ ){ 
            ...
            ?>
            <section class="<?php echo $class; ?> <?php if( of_get_option('sectioncontent1',true) == 1) {?>hidden<?php } ?>" <?php if($bgcolor!=''){?>style="background-color:<?php echo $bgcolor;?>;"<?php } ?>>
                <div class="container">
                    <?php if( $title != '' ) { ?>
	...
	if ( have_posts() ) {
		while ( have_posts() ) { 
			...
			<div class="one_half"><div class="post-meta"><?php echo get_the_time('j'); ?><span><?php echo get_the_date('M'); ?></span></div><div class="post-next"><h4 class="blog-title"><a href="<?php esc_url(the_permalink()); ?>" title="<?php the_title(); ?>">'<?php the_title(); ?></a></h4>
	...
	<?php if ( have_posts() ) {
		while ( have_posts() ) { 
			...
			<div class="post-meta-small"><?php echo get_the_time('j'); ?><span><?php echo get_the_date('M'); ?></span></div><div class="post-next"><h4 class="blog-title"><a href="<?php esc_url(the_permalink()); ?>" title="<?php  the_title(); ?>"><?php the_title(); ?></a></h4>
